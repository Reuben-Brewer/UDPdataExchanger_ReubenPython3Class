...
class UDPdataExchanger_ReubenPython3Class(Frame): #Subclass the Tkinter Frame
    ...
    ##########################################################################################################
    ##########################################################################################################
    def ConvertJSONstringToDict(self, JSONstringToConvert, PrintInfoForDebuggingFlag = 0):
        ...
        try:
            if type(JSONstringToConvert) == str:
                ...
                if PrintInfoForDebuggingFlag: print("ConvertJSONstringToDict: JSONstringToConvert = " + JSONstringToConvert + ", DictToConvert = " + str(DictToConvert))
    ...
    ##########################################################################################################
    ##########################################################################################################
    def RxUDPmessage(self, PrintInfoForDebuggingFlag = 0):
        ...
        try:
            ...
            if self.IPV4_Port >= 1 and self.IPV4_Port <= 65535: #Can't be 0
                ...
                if self.UDP_PortIsOpenFlag == 1:
                    ...
                    Rx_DataDict = self.ConvertJSONstringToDict(Rx_DataStr, PrintInfoForDebuggingFlag)
    ...
    ##########################################################################################################
    ##########################################################################################################
    def SendDictFromExternalProgram(self, InputDict, PrintInfoForDebuggingFlag=0):
        ...
        try:
            ...
            if self.UDP_RxOrTxRole == "tx":
                ...
                if PrintInfoForDebuggingFlag == 1: print("SendDictFromExternalProgram: len(JSONstringToTx) = " + str(len(JSONstringToTx)) + ", JSONstringToTx = " + JSONstringToTx)
                ...
                if self.UDP_RxOrTxRole == "rx":
                    ...
                    try:
                        ...
                        if self.DataStream_State == 1:
                            self.RxUDPmessage(PrintInfoForDebuggingFlag=0)
